...
#define DOCTEST_DO_BINARY_EXPRESSION_COMPARISON(op, op_str, op_macro)                         \
    ...
    template <typename R, typename enable_if<!doctest::detail::is_rvalue_reference<R>::value, \
                                             void>::type* = nullptr>                          \
    DOCTEST_NOINLINE SFINAE_OP(Result, op) operator op(const R& rhs) {                        \
...
struct DOCTEST_INTERFACE ExpressionDecomposer {
    ...
    template <typename L, typename enable_if<!doctest::detail::is_rvalue_reference<L>::value,
                                             void>::type* = nullptr>
    Expression_lhs<const L&> operator<<(const L& operand) {
...
class ContextScope : public ContextScopeBase {
   ...
   public:
    ...
    ContextScope(ContextScope&& other)
        : ContextScopeBase(static_cast<ContextScopeBase&&>(other)), lambda_(other.lambda_) {}
...
struct DOCTEST_INTERFACE IReporter {
    ...
    // doctest will not be managing the lifetimes of reporters given to it but this would still be
    // nice to have
    virtual ~IReporter();
...
struct ConsoleReporter : public IReporter {
    ...
    void test_run_end(const TestRunStats& p) override {
        ...
        auto totwidth = int(std::ceil(log10(
            (std::max(p.numTestCasesPassingFilters, static_cast<unsigned>(p.numAsserts))) + 1)));
        auto passwidth = int(
            std::ceil(log10((std::max(p.numTestCasesPassingFilters - p.numTestCasesFailed,
                                      static_cast<unsigned>(p.numAsserts - p.numAssertsFailed))) +
                            1)));
        auto failwidth = int(std::ceil(log10(
            (std::max(p.numTestCasesFailed, static_cast<unsigned>(p.numAssertsFailed))) + 1)));
