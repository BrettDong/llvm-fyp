...
void CallGraphPass::findCalleesWithType(CallInst *CI, FuncSet &S) {
    ...
    for (Function *F : Ctx->AddressTakenFuncs) {
        ...
        for (Function::arg_iterator FI = F->arg_begin(), FE = F->arg_end(); FI != FE; ++FI, ++AI) {
            ...
            if (DefinedTy == ActualTy) continue;
            ...
            if ((DefinedTy == Int8PtrTy && (ActualTy->isPointerTy() || ActualTy == IntPtrTy)) ||
                (ActualTy == Int8PtrTy && (DefinedTy->isPointerTy() || DefinedTy == IntPtrTy)))
                continue;
            else {
                Matched = false;
                break;
