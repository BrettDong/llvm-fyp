...
void VirtualCallTargetsResult::addVirtualCallCandidates(CallInst *call, FunctionSet &&candidates) {
    addCandidates(call, move(candidates));
}

void VirtualCallTargetsResult::addVirtualInvokeCandidates(InvokeInst *call,
                                                          FunctionSet &&candidates) {
    addCandidates(call, move(candidates));
}

bool VirtualCallTargetsResult::hasVirtualCallCandidates(Instruction *instr) const {
    return m_virtualCallCandidates.find(instr) != m_virtualCallCandidates.end();
}

const FunctionSet &VirtualCallTargetsResult::getVirtualCallCandidates(Instruction *instr) const {
...
void VirtualCallTargetsResult::addCandidates(Instruction *instr, FunctionSet &&candidates) {
...
class VirtualCallTargets : public llvm::AnalysisInfoMixin<VirtualCallTargets> {
   ...
   private:
    ...
    using VTableSlotCallSitesMap =
        std::unordered_map<VTableSlot, VirtualCallSites, VTableSlotHasher, VTableSlotEqual>;

    void scanTypeTestUsers(llvm::Function *TypeTestFunc);
    void buildTypeIdentifierMap(
        llvm::Module &M, std::vector<llvm::wholeprogramdevirt::VTableBits> &Bits,
        llvm::DenseMap<llvm::Metadata *, std::set<llvm::wholeprogramdevirt::TypeMemberInfo>>
            &TypeIdMap);
    bool tryFindVirtualCallTargets(
        llvm::Module &M, std::vector<llvm::wholeprogramdevirt::VirtualCallTarget> &TargetsForSlot,
...
void VirtualCallTargets::scanTypeTestUsers(Function *TypeTestFunc) {
    ...
    // to CallSlots.
    for (auto I = TypeTestFunc->use_begin(), E = TypeTestFunc->use_end(); I != E;) {
        ...
        Metadata *TypeId = cast<MetadataAsValue>(CI->getArgOperand(1))->getMetadata();
...
void VirtualCallTargets::buildTypeIdentifierMap(
    ...
    for (GlobalVariable &GV : M.globals()) {
        ...
        if (GV.isDeclaration() || Types.empty()) continue;
        ...
        for (MDNode *Type : Types) {
            ...
            uint64_t Offset =
                cast<ConstantInt>(cast<ConstantAsMetadata>(Type->getOperand(0))->getValue())
                    ->getZExtValue();
...
bool VirtualCallTargets::tryFindVirtualCallTargets(
    ...
    const set<wholeprogramdevirt::TypeMemberInfo> &TypeMemberInfos, uint64_t ByteOffset) {
    for (const wholeprogramdevirt::TypeMemberInfo &TM : TypeMemberInfos) {
        if (!TM.Bits->GV->isConstant()) return false;

        Constant *Ptr =
            getPointerAtOffset(TM.Bits->GV->getInitializer(), TM.Offset + ByteOffset, M);
        if (!Ptr) return false;
        ...
        if (Fn->getName() == "__cxa_pure_virtual") continue;

        LOG_FMT("%s ==> %s\n", Fn->getName().str().c_str(), TM.Bits->GV->getName().str().c_str());
...
VirtualCallTargetsResult VirtualCallTargets::runOnModule(Module &M) {
    ...
    if ((!TypeTestFunc || TypeTestFunc->use_empty() || !AssumeFunc || AssumeFunc->use_empty())) {
...
VirtualCallTargetsResult VirtualCallTargets::run(Module &M, ModuleAnalysisManager &AM) {
...
void VirtualCallResolver::ResolveVirtualCalls(Module &M, VirtualCallTargetsResult &result) {
    //
    ...
    MPM.addPass(move(VirtualWrapper));
